...
// ==================== Notion 配置（從 Script Properties 讀取）=================
function getNotionConfig() {
  const scriptProperties = PropertiesService.getScriptProperties();
  return {
    token: scriptProperties.getProperty('notion_token') || '',
    databaseId: scriptProperties.getProperty('notion_database_id') || '311570d9c301804e8b49e4f2cb9f60cb'
  };
}

// 設定 Notion API Token（首次設定用）
function setupNotionToken() {
  // 請在 Google Apps Script 中手動設定 Script Properties:
  // 點擊 [檔案] → [專案設定] → [指令碼屬性]，新增 notion_token
  return '請在 Script Properties 中設定 notion_token';
}

// ==================== 儲存客戶資料到 Notion ====================
function saveToNotion(quoteData) {
  try {
    const NOTION_CONFIG = getNotionConfig();
    
    if (!NOTION_CONFIG.token) {
      return { success: false, error: '尚未設定 Notion API Token。請在 Script Properties 中設定 notion_token' };
    }
    ...
    const payload = {
      ...
      properties: {
        '客戶編號': { title: [{ text: { content: 'C' + Date.now() } }] },
        '姓名': { rich_text: [{ text: { content: quoteData.customer.name } }] },
        '電話': { phone_number: quoteData.customer.phone },
        '安裝地址': { rich_text: [{ text: { content: quoteData.customer.address } }] },
        '建立日期': { date: { start: new Date().toISOString().split('T')[0] } },
        '報價金額': { number: quoteData.total || null }
      }
    };
    ...
    if (response.getResponseCode() === 200) {
      ...
      return { success: false, error: 'Notion API error: ' + response.getResponseCode() };
    }
